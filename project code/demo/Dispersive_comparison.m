...
%% Main Loop
% Define the modes to run: 'sm' = SM-INSAF, 'ssm' = SSM-INSAF, 'full' = fullband INSAF
types = {'sm', 'ssm', 'full'};
...
for mode = types
    ...
    upd_cnt   = zeros(Nsub,1);              % Counter of updates per subband
        ...
        for i = 1:Nsub
           ...
           if strcmp(tag, "sm")
                if abs(e_i) <= gamma_i(i)
                    do_update = false;        % skip update when error within bound
                else
                    % variable step-size for SM-INSAF
                    mu_i = 1 - gamma_i(i)/max(abs(e_i),1e-12);
                end


            elseif strcmp(tag, "ssm")
                % update smoothed error estimate
                sigma_eps(i) = beta * sigma_eps(i) + (1 - beta) * abs(e_i);

                if min(abs(e_i), sigma_eps(i)) <= gamma_i(i)
                    do_update = false;        % skip update when both instant and smoothed errors small
                else
                    % variable step-size using smoothed error
            ...
            % Perform the weight update contribution for this subband
            if do_update
                % compute normalized subband update: (mu0 * mu_i * e_i * uvec)/(energy + delta)
                dW = dW + (mu0 * mu_i * e_i * uvec) / (uvec' * uvec + delta);
                upd_cnt(i) = upd_cnt(i) + 1;   % record that subband i was updated
            end
        end

        % Form the new weight vector by adding the total increment
        w_new   = wbar + dW;

        % Shift the weight history: newest w_new at column 1, drop oldest
...
title('INSAF Variants (Pseudo-Downsampled )');
grid on; ylim([-50 5]);
